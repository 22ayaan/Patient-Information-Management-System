...
class Database:
    def __init__(self):
        self.mycon = sqlite3.connect("patientdb.db")
        self.cur = self.mycon.cursor()
        self.cur.execute("CREATE TABLE IF NOT EXISTS patient_table (id PRIMARYKEY text, firstname text, lastname text, dateOfBirth text, monthOfBirth text, yearOfBirth text, gender text, address text, contactNumber text, emailAddress text, bloodType text, history text, doctor text)")
    ...
    def Insert(self, id, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress, bloodType, history, doctor):
        self.cur.execute("INSERT INTO patient_table VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (id, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress, bloodType, history, doctor))
        self.mycon.commit()
    ...
    def Update(self, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress, bloodType, history, doctor, id):
        self.cur.execute("UPDATE patient_table SET firstname = ?, lastname = ?, dateOfBirth = ?, monthOfBirth = ?, yearOfBirth = ?, gender = ?, address = ?, contactNumber = ?, emailAddress = ?, bloodType = ?, history = ?, doctor = ? WHERE id = ?",(firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber, emailAddress, bloodType, history, doctor, id))
        self.mycon.commit()
 
    def Search(self, id):
        self.cur.execute("SELECT * FROM patient_table WHERE id = ?", (id,))
        searchResults = self.cur.fetchall()
        return searchResults
 
    def Delete(self, id):
        self.cur.execute("DELETE FROM patient_table WHERE id = ?", (id,))
        self.mycon.commit()
 
    def Display(self):
        self.cur.execute("SELECT * FROM patient_table")
...
class Values:
    def Validate(self, id, firstname, lastname, contactNumber, emailAdress, history, doctor):
        ...
        elif not (emailAdress.count("@") == 1 and emailAdress.count(".") == 1):
            return "emailAddress"
...
class InsertWindow:
    def __init__(self):
        ...
        self.monthType = ["January", "February", "March", "April", "May", "June", "July", "August", "September","October", "November", "December"]
        self.yearType = list(range(1960, 2022))
...
class UpdateWindow:
    def __init__(self, id):
        ...
        # Button
        tkinter.Button(self.window, width=10, fg=fg_color, bg=bg_color, font=("times new roman",10,"bold"), text="Update", command=self.Update).grid(pady=15, padx=5, column=1,row=14)
        tkinter.Button(self.window, width=10, fg=fg_color, bg=bg_color, font=("times new roman",10,"bold"), text="Reset", command=self.Reset).grid(pady=15, padx=5, column=2, row=14)
        tkinter.Button(self.window, width=10, fg=fg_color, bg=bg_color, font=("times new roman",10,"bold"), text="Close", command=self.window.destroy).grid(pady=15, padx=5, column=3,row=14)
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView["columns"] = ("id", "firstname", "lastname", "dateOfBirth", "monthOfBirth", "yearOfBirth", "gender", "address", "contactNumber", "emailAddress", "bloodType", "history","doctor")
